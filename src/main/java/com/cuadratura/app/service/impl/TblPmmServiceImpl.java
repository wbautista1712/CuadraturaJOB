...
public class TblPmmServiceImpl extends GenericServiceImpl<TblPmm, Integer> implements TblPmmService {
	...
	@Override
	public void saveTblPmmJob(List<TblHstFtFapinvbaleeDto> listaTblPmmForm, int idCargaPMM) throws SQLException {
		LOGGER.info("> listaTblPmmForm", listaTblPmmForm.size());
		TblPmm statement = null;
	
		for (TblHstFtFapinvbaleeDto obj : listaTblPmmForm) {
			statement = new TblPmm();
		
			// org_lvl_child, prd_lvl_child, inv_type_code, trans_lote, on_hand_qty,
			statement.setOrgLvlChild(obj.getOrgLvlChild().intValue());
			statement.setPrdLvlChild(obj.getPrdLvlChild().intValue());
			statement.setInvTypeCode(obj.getInvTypeCode()== null ? null : obj.getInvTypeCode().trim());
			statement.setTransLote(obj.getTransLote()== null ? null : obj.getTransLote().trim());
			...
			statement.setAuditNumber(obj.getAuditNumber().intValue());

			this.tblPmmRepository.saveTblPmmJob(statement, idCargaPMM);
		}
